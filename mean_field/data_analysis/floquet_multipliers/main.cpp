...
int main(int argc, char* argv[]) {
	if (argc < 6) {
		std::cout << "Required arguments: points a0 b0 k0 k1_step" << std::endl;
		return 0;
	}
	...
	ps[1] = stod(argv[3]); // b0
	ps[2] = stod(argv[4]); // k0
	const double k1_step = stod(argv[5]);
	...
	double t_end;
	double dt;
	std::ofstream output("output/a0="+(string)argv[2]+"_b0="+(string)argv[3]+"_k0="+(string)argv[4]+".dat");
