...
void ETD2_solver::calculate_coeffcients_log() {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      f2[i * N + j] =
          c_u[i * N + j] == 0 ? dt : (f1[i * N + j] - 1) / c_u[i * N + j];
      f3[i * N + j] = c_u[i * N + j] == 0
                          ? 1.5 * dt
                          : ((1 + c_u[i * N + j] * dt) * f1[i * N + j] - 1 -
                             2 * c_u[i * N + j] * dt) /
                                (std::pow(c_u[i * N + j], 2) * dt);
      f4[i * N + j] = c_u[i * N + j] == 0
                          ? -0.5 * dt
                          : (-f1[i * N + j] + 1 + c_u[i * N + j] * dt) /
                                (std::pow(c_u[i * N + j], 2) * dt);
      c_v[i * N + j] = k2_v;
      g1[i * N + j] = exp(k2_v * dt);
      g2[i * N + j] =
          c_v[i * N + j] == 0 ? dt : (g1[i * N + j] - 1) / c_v[i * N + j];
      g3[i * N + j] = c_v[i * N + j] == 0
                          ? 1.5 * dt
                          : ((1 + c_v[i * N + j] * dt) * g1[i * N + j] - 1 -
                             2 * c_v[i * N + j] * dt) /
                                (std::pow(c_v[i * N + j], 2) * dt);
      g4[i * N + j] = c_v[i * N + j] == 0
                          ? -0.5 * dt
                          : (-g1[i * N + j] + 1 + c_v[i * N + j] * dt) /
                                (std::pow(c_v[i * N + j], 2) * dt);
    }
  }
}

void ETD2_solver::calculate_coeffcients_regular() { // Need to adjust this for
                                                    // now mu depends on time
  double k2_u;
  double k2_v;
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
...
void ETD2_solver::complex_multiply(fftw_complex z, fftw_complex w,
                                   fftw_complex result) {
...
void ETD2_solver::calculate_non_linear_log(double t) {
  fftw_complex *deriv_u_x =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * N * N);
  fftw_complex *deriv_v_x =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * N * N);
  fftw_complex *deriv_u_y =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * N * N);
  fftw_complex *deriv_v_y =
      (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * N * N);
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      f[i * N + j][0] = Dx_u * std::pow(deriv_u_x[i * N + j][0], 2) +
                        Dy_u * std::pow(deriv_u_y[i * N + j][0], 2) +
                        lambda(t) * exp(v[i * N + j][0]) - mu(t);
      g[i * N + j][0] =
          Dx_v * std::pow(deriv_v_x[i * N + j][0], 2) +
          Dy_v * std::pow(deriv_v_y[i * N + j][0], 2) -
          lambda(t) * exp(u[i * N + j][0]) +
          (1 - k(t) * exp(u[i * N + j][0]) - k(t) * exp(v[i * N + j][0]));
      f[i * N + j][1] = 0;
      g[i * N + j][1] = 0;
    }
  }
...
void ETD2_solver::calculate_non_linear_regular() {
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      g[i * N + j][0] =
          -lambda(0) * v[i * N + j][0] * u[i * N + j][0] -
          (u[i * N + j][0] * v[i * N + j][0] + std::pow(v[i * N + j][0], 2));
      f[i * N + j][1] = 0;
      g[i * N + j][1] = 0;
    }
  }
...
void ETD2_solver::set_initial_conditions(int type) {
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      if (type ==
          0) { // Checkboard pattern (need to make type names more transparent)
        if (xx[i] < (double)L / 2 && yy[j] < (double)L / 2) {
          ...
          v[i * N + j][1] = 0;
                 ...
                 1) { // Continuous initial conditions with sines and cosines
                      // (need to make type names more transparent)
        u[i * N + j][0] = exp(sin(2 * M_PI * xx[i] / L));
        u[i * N + j][1] = 0;
        v[i * N + j][0] = exp(cos(2 * M_PI * yy[j] / L));
        v[i * N + j][1] = 0;
      } else if (type == 2) { // Constant initial conditions (need to make type
...
void ETD2_solver::time_step(bool first_time) {
  if (first_time) {
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        u[i * N + j][0] =
            f1[i * N + j] * u[i * N + j][0] + f2[i * N + j] * f[i * N + j][0];
        u[i * N + j][1] =
            f1[i * N + j] * u[i * N + j][1] + f2[i * N + j] * f[i * N + j][1];
        v[i * N + j][0] =
            g1[i * N + j] * v[i * N + j][0] + g2[i * N + j] * g[i * N + j][0];
        v[i * N + j][1] =
            g1[i * N + j] * v[i * N + j][1] + g2[i * N + j] * g[i * N + j][1];
